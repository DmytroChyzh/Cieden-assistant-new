...
export function Component() {
  ...
  // For subtle 3D card effect - much reduced rotation
  const mouseX = useMotionValue(0);
  const mouseY = useMotionValue(0);
  const rotateX = useTransform(mouseY, [-300, 300], [2, -2]); // Reduced from 10/-10 to 2/-2
  const rotateY = useTransform(mouseX, [-300, 300], [-2, 2]); // Reduced from -10/10 to -2/2
    ...
    try {
      ...
      if (error instanceof Error) {
        ...
        if (error.message.includes("InvalidAccountId")) {
          alert("Account not found. Please sign up first or check your email address.");
        } else if (error.message.includes("InvalidSecret") || error.message.includes("Uncaught Error: InvalidSecret")) {
          alert("Invalid password. Please check your credentials.");
        } else if (error.message.includes("expired") || error.message.includes("token")) {
          alert("Session expired. Please try signing in again.");
        } else {
              ...
              <div className="relative bg-black/40 backdrop-blur-xl rounded-2xl p-6 border border-white/[0.05] shadow-2xl overflow-hidden">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <motion.button
                    type="button"
                    disabled={true}
                    className="w-full relative group/google opacity-50 cursor-not-allowed"
                  >
                    <div className="relative overflow-hidden bg-white/5 text-white font-medium h-10 rounded-lg border border-white/10 transition-all duration-300 flex items-center justify-center gap-2">
