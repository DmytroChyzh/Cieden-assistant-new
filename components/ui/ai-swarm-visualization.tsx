...
export const Component = () => {
  ...
  // Reduced default speed so the animation runs a bit slower
    ...
    const initParticles = () => {
      ...
      for (let i = 0; i < particleCount; i++) {
        particles.push({
          ...
          connect: [],
          // Reduce particle opacity by ~30%
          opacity: (Math.random() * 0.5 + 0.5) * 0.7
        });
      }
      ...
      for (let i = 0; i < particlesRef.current.length; i++) {
        ...
        for (let j = i + 1; j < particlesRef.current.length; j++) {
          ...
          if (distance < connectionDistance) {
            // Calculate opacity based on distance and reduce by 30%
