...
export function EstimateFinalResultSidePanel({ result, onClose }: EstimateFinalResultSidePanelProps) {
  ...
  return (
    ...
    >
      ...
      {showContactFull ? (
        <div className="flex-1 flex flex-col min-h-0 overflow-y-auto overflow-x-hidden px-5 py-4 scrollbar-chat">
        ...
        <div className="flex-1 overflow-y-auto overflow-x-hidden px-5 py-4 scrollbar-chat">
          <div className="rounded-2xl border border-indigo-400/20 bg-gradient-to-br from-indigo-500/[0.12] to-violet-600/[0.08] backdrop-blur-md p-5 shadow-[0_0_32px_rgba(99,102,241,0.12),inset_0_1px_0_rgba(255,255,255,0.06)]">
            ...
            <div className="mt-5 space-y-2">
              <p className="text-[11px] uppercase tracking-wider text-white/40 font-semibold">Work included</p>
              <div className="grid grid-cols-2 gap-3">
                {PHASE_ORDER.map(({ label, Icon }) => (
                  <div
                    key={label}
                    className="rounded-2xl border border-indigo-400/15 bg-gradient-to-br from-indigo-500/10 to-violet-600/[0.06] p-3 flex items-center gap-3"
                  >
                    <span className="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-indigo-500/15 border border-indigo-400/20 text-indigo-300">
                      <Icon className="w-4 h-4" aria-hidden />
                    </span>
                    <p className="text-[12px] font-semibold text-white/90 leading-snug">{label}</p>
                  </div>
                ))}
              </div>
            </div>
